        ...
        if (WSResults) {
            WSResults.map(ws => {
                ...
                const devToMedian = ws.data.parameters.map(param => {
                    ...
                    else if (param.value===0 || medianParams[param.index][0]===0) { 
                        return 100
                    }
                    else {
                        return Math.round(Math.abs(param.value - medianParams[param.index][0])/medianParams[param.index][0]*100,1)
                    }   
                })
                ...
                //get nbModifs
                ws.nbModifs =  ws.data.parameters.filter(param => param.value!==medianParams[param.index][0]).length 
            ...
            const tableTemp = {
                ...
                "data" : WSResults.map(ws => [ws.group_name, ws.nbModifs, ws.nbResults, Math.round(ws.nbModifs/ws.nbResults*100,0) + "%", ws.average])
            }
